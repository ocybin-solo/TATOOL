...
var active_pass1_shader_name: String = "None Selected"
...
func setup_dual_pass_pipeline() -> void:
	...
	# --- PASS 3: POST-PROCESS FILTER BUFFER ---
	...
	# ✅ PARALLEL BINDINGS (Fixed black screen bug)
...
func setup_interface_layer() -> void:
	...
	# ✅ 🤫 HIDDEN SCREENSAVER PLACEHOLDER (Replaces legacy export button)
	...
	# ✅ RENAME: Shader Menu Integration
	var btn_start_menu = Button.new()
	btn_start_menu.text = "🕹️ SHADER MENU"
	btn_start_menu.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	btn_start_menu.pressed.connect(_on_shader_menu_button_pressed)
...
func _on_select_button_pressed() -> void:
	if is_menu_open: 
		print("⚠️ UI Input Blocked: Cannot switch layers while Shader Menu is active.")
		return
...
func open_shader_menu() -> void:
	...
	if active_shader_layer == 0:
		active_menu_categories = pass1_library.keys() 
...
func close_shader_menu() -> void:
	...
	if not active_menu_categories.is_empty() and active_menu_index < active_menu_categories.size():
		...
		match active_shader_layer:
			0:
				p1_saved_index = 0
				p1_saved_sub_channel = 0
			1:
				p2_saved_index = 0
				p2_saved_sub_channel = 0
			2:
				p3_saved_index = 0
				p3_saved_sub_channel = 0
...
func _process(delta: float) -> void:
	...
	# ✅ CONCURRENT CLOCK SETS (Syncs all timeline variables dynamically)
	...
	if current_vram_mb > vram_safety_trip_point or current_fps < 30.0:
		if pass1_material:
			...
			if active_detail != null and active_detail > 2.0:
				...
				control_panel.update_status_readout()
				label_safety_alert.text = "⚠️ PERFORMANCE OVERLOAD PREVENTED: CLAMPING NOISE"
...
func load_default_test_shaders() -> void:
	...
	void fragment() {
		vec2 uv = UV;
		vec4 center_color = texture(u_pattern_texture, uv);
		float c  = (center_color.r + center_color.g + center_color.b) / 3.0;
		float left  = (texture(u_pattern_texture, uv - vec2(u_step_offset.x, 0.0)).g);
		float right = (texture(u_pattern_texture, uv + vec2(u_step_offset.x, 0.0)).g);
		float up    = (texture(u_pattern_texture, uv - vec2(0.0, u_step_offset.y)).g);
		float down  = (texture(u_pattern_texture, uv + vec2(0.0, u_step_offset.y)).g);
	...
	var s1 = Shader.new()
	s1.code = PASSTHROUGH_SHADER_CODE
	var s2 = Shader.new()
...
func _on_global_factory_reset_pressed() -> void:
	...
	print("⚠️ Master Reset: Wiping parameters to defaults.")
